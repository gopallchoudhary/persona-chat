...
More about yourself:
    ...
    Output JSON Format:
    { "step": "ANALYSE | THINK | VALIDATE | OUTPUT | RESULT", "content": "string" }
...
`,
        ...
        try {
            ...
            if (parsed.step?.toUpperCase() === 'RESULT' || parsed.step?.toUpperCase() === 'OUTPUT') {
                parsedContent = parsed.content;
            } else {
                parsedContent = parsed.content ?? raw;
            }
        } catch {
            parsedContent = raw; // fallback if it's already plain text
        }
